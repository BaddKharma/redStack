...
└── WIN-ATTACKER - Windows 11 Workstation (internal only - RDP via Guacamole)
...
- Windows (`WIN-ATTACKER`): Disabling Defender/Firewall, enabling RDP, installing dev tools (VS Code, Chrome, MobaXterm, VS Build Tools), WSL Debian, and Windows Terminal
- Redirector (`redirector`): Configuring SSH access only (Debian 12, Apache setup is manual — see Step 2.4)
...
1. **Windows 11 Attacker Workstation** (RDP) - auto-connects with Administrator credentials
...
**Expected Output:** `win-attacker\administrator`
...
**From the Windows attacker workstation (via Guacamole RDP):**
